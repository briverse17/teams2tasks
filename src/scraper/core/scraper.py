...
class TeamsScraper:
    ...
    async def scrape(
        self, mode: str = "default", targets: List[str] = None, stop_early: bool = False
    ):
        async with async_playwright() as p:
            ...
            if not self.auth_path.exists():
                try:
                    ...
                    logger.info(f"Auth saved to {self.auth_path}")
                except Exception as e:
            ...
            for chat_el in chat_elements:
                    ...
                    if sidebar_date:
                        if from_date and to_date:
                            if from_date <= sidebar_date <= to_date:
                                logger.info(
                                    f"Chat '{chat_name}' active on {sidebar_date}. Processing..."
                                )
                                should_process = True
                            elif sidebar_date < from_date:
                                if stop_early:
                                    logger.info(
                                        f"Chat '{chat_name}' active on {sidebar_date} (too old). Stopping early."
                                    )
                                    break
                                else:
                                    logger.debug(
                                        f"Chat '{chat_name}' active on {sidebar_date} (too old). Skipping."
                                    )
                                    should_process = False
                            else:
                                logger.debug(
                                    f"Chat '{chat_name}' active on {sidebar_date} (too new). Skipping."
                                )
                                should_process = False
                        else:
                            should_process = True
                        ...
                        if potential_dates:
                            ...
                            if in_range:
                                logger.info(
                                    f"Chat '{chat_name}' has ambiguous date(s) {potential_dates}. Interpretations {in_range} are in range. Opening..."
                                )
                                should_process = True
                            else:
                                logger.debug(
                                    f"Chat '{chat_name}' has ambiguous date(s) {potential_dates}. None are in range. Skipping."
                                )
                                should_process = False
                        else:
                            # Final fallback: open to verify
                            logger.debug(
                                f"Could not parse sidebar date for '{chat_name}'. Opening to verify."
                            )
                            should_process = True
                ...
                try:
                    ...
                    if chat_messages:
                        chats_data.append(Chat(id=chat_id, name=chat_name, messages=chat_messages))
                except Exception as e:
